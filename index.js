...
app.get('/api/payment-methods/:itemId', (req, res) => {
  ...
  // Add internal balance payment method
  paymentMethods.push({
    id: 'BALANCE',
    name: 'Balance',
    icon: 'https://i.postimg.cc/3N3f5zhH/IMG-1243.png', // Using Stars icon as a placeholder
    price: item.price, // Assuming 'price' is the cost in internal currency
    contact: '@MetaGift_support'
  });
...
app.post('/api/buy', async (req, res) => {
  ...
  try {
    ...
    await updateUserBalance(userId, userBalance - item.price);

    // Add item to user's inventory (simplified)
    const inventory = loadInventoryData();
    inventory.push({ userId, itemId, name: item.name, image: item.image });
